...
export const getAllMemos = (currentUserId?: number, { state = ROW_STATUS.NORMAL } = {}) => {
  const filterList = ["order_by_pinned == true"];
  if (currentUserId) {
    filterList.push(`creator == "users/${currentUserId}"`);
  }
  if (state !== ROW_STATUS.ALL) {
    filterList.push(`row_status == "${state}"`);
  }
  const filter = filterList.join(" && ");

  const url = getRequestUrl(`/api/v1/memos?filter=${encodeURIComponent(filter)}&pageSize=20`);
